...
import cn.utsoft.commons.video.UTBaseVideoPlayer;
...
public abstract class UTVideoPlayer extends UTBaseVideoPlayer implements View.OnClickListener, View.OnTouchListener, SeekBar.OnSeekBarChangeListener, TextureView.SurfaceTextureListener {
    ...
    public UTVideoPlayer(Context context) {
        super(context);
        init(context);
    }

    public UTVideoPlayer(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context);
    }
     ...
     */
    public UTVideoPlayer(Context context, Boolean fullFlag) {
    ...
    public boolean setUp(String url, boolean cacheWithPlay, File cachePath, Object... objects) {
        ...
        if (cacheWithPlay && url.startsWith("http") && !url.contains("127.0.0.1")) {
            mOriginUrl = url;
            HttpProxyCacheServer proxy = UTVideoManager.getProxy(getContext().getApplicationContext(), cachePath);
            url = proxy.getProxyUrl(url);
            mCacheFile = (!url.startsWith("http"));
            if (!mCacheFile && UTVideoManager.instance() != null) {
                proxy.registerCacheListener(UTVideoManager.instance(), mOriginUrl);
            }
        }
        this.mUrl = url;
    ...
    public void onClick(View v) {
        ...
        if (i == R.id.startbutton) {
            ...
            if (mCurrentState == CURRENT_STATE_NORMAL || mCurrentState == CURRENT_STATE_ERROR) {
                ...
                UTVideoManager.instance().getMediaPlayer().pause();
                ...
                UTVideoManager.instance().getMediaPlayer().start();
    ...
    protected void prepareVideo() {
        if (UTVideoManager.instance().listener() != null) {
            UTVideoManager.instance().listener().onCompletion();
        }
        UTVideoManager.instance().setListener(this);
        UTVideoManager.instance().setPlayTag(mPlayTag);
        UTVideoManager.instance().setPlayPosition(mPlayPosition);
        addTextureView();
        mAudioManager.requestAudioFocus(onAudioFocusChangeListener, AudioManager.STREAM_MUSIC, AudioManager.AUDIOFOCUS_GAIN_TRANSIENT);
        ((Activity) getContext()).getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
        UTVideoManager.instance().prepare(mUrl, mMapHeadData, mLooping, mSpeed);
        ...
        public void onAudioFocusChange(int focusChange) {
            switch (focusChange) {
                ...
                case AudioManager.AUDIOFOCUS_LOSS_TRANSIENT:
                    if (UTVideoManager.instance().getMediaPlayer().isPlaying()) {
                        UTVideoManager.instance().getMediaPlayer().pause();
                    }
                    break;
    ...
    public void onVideoPause() {
        if (UTVideoManager.instance().getMediaPlayer().isPlaying()) {
            ...
            mCurrentPosition = UTVideoManager.instance().getMediaPlayer().getCurrentPosition();
            if (UTVideoManager.instance().getMediaPlayer() != null)
                UTVideoManager.instance().getMediaPlayer().pause();
        }
    }
    ...
    public void onVideoResume() {
        ...
        if (mCurrentState == CURRENT_STATE_PAUSE) {
            if (mCurrentPosition > 0 && UTVideoManager.instance().getMediaPlayer() != null) {
                setStateAndUi(CURRENT_STATE_PLAYING);
                UTVideoManager.instance().getMediaPlayer().seekTo(mCurrentPosition);
                UTVideoManager.instance().getMediaPlayer().start();
            }
        }
    }
    ...
    protected void addTextureView() {
        ...
        mTextureView = null;
        mTextureView = new UTTextureView(getContext());
    ...
    public void onSurfaceTextureAvailable(SurfaceTexture surface, int width, int height) {
        ...
        UTVideoManager.instance().setDisplay(mSurface);
        //显示暂停切换显示的图片
        showPauseCover();
    }
    ...
    public boolean onSurfaceTextureDestroyed(SurfaceTexture surface) {
        UTVideoManager.instance().setDisplay(null);
        surface.release();
        cancelProgressTimer();
        return true;
    }
        ...
        if (id == R.id.surface_container) {
            switch (event.getAction()) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (mIfCurrentIsFullscreen || mIsTouchWiget) {// TODO: 2016/12/20 修改为只有在全屏的时候才可以修改亮度等
                    ...
                    if (mChangePosition) {
                        ...
                        mSeekTimePosition = (int) (mDownPosition + deltaX * 100000 / mScreenWidth);// TODO: 2016/12/20 滑动全屏幕的时候只能快进100秒
                        if (mSeekTimePosition > totalTimeDuration)
                            mSeekTimePosition = totalTimeDuration;
                        String seekTime = CommonUtil.stringForTime(mSeekTimePosition);
                        String totalTime = CommonUtil.stringForTime(totalTimeDuration);
                        showProgressDialog(deltaX, seekTime, mSeekTimePosition, totalTime, totalTimeDuration);
                    } else if (mChangeVolume && mIfCurrentIsFullscreen) {
//                        deltaY = -deltaY;
...
//                        //降低音量，调出系统音量控制
                        ...
                        } else if (deltaY > 0) {//增加音量，调出系统音量控制
                            mAudioManager.adjustStreamVolume(AudioManager.STREAM_MUSIC, AudioManager.ADJUST_LOWER,
                                    AudioManager.FX_FOCUS_NAVIGATION_UP);
                        }
//                        showVolumeDialog(-deltaY, volumePercent);
                    } else if (!mChangePosition && mBrightness && mIfCurrentIsFullscreen) {
...
//                    dismissBrightnessDialog();
                    if (mChangePosition) {
                        UTVideoManager.instance().getMediaPlayer().seekTo(mSeekTimePosition);
    ...
    public void onStopTrackingTouch(SeekBar seekBar) {
        ...
        if (UTVideoManager.instance().getMediaPlayer() != null && mHadPlay) {
            int time = seekBar.getProgress() * getDuration() / 100;
            UTVideoManager.instance().getMediaPlayer().seekTo(time);
        }
    }
    ...
    public void onPrepared() {
        ...
        if (UTVideoManager.instance().getMediaPlayer() != null && mSeekToInAdvance != -1) {
            UTVideoManager.instance().getMediaPlayer().seekTo(mSeekToInAdvance);
            mSeekToInAdvance = -1;
        }
        ...
        if (UTVideoManager.instance().getMediaPlayer() != null && mSeekOnStart > 0) {
            UTVideoManager.instance().getMediaPlayer().seekTo(mSeekOnStart);
            mSeekOnStart = 0;
        }
    ...
    public void onAutoCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {
            ...
            if (UTVideoManager.instance().lastListener() != null) {
                UTVideoManager.instance().lastListener().onAutoCompletion();
            }
        }
        if (!mIfCurrentIsFullscreen)
    ...
    public void onCompletion() {
        ...
        if (IF_FULLSCREEN_FROM_NORMAL) {//如果在进入全屏后播放完就初始化自己非全屏的控件
            IF_FULLSCREEN_FROM_NORMAL = false;
            if (UTVideoManager.instance().lastListener() != null) {
                UTVideoManager.instance().lastListener().onCompletion();//回到上面的onAutoCompletion
            }
        }
        if (!mIfCurrentIsFullscreen) {
            UTVideoManager.instance().setListener(null);
            UTVideoManager.instance().setLastListener(null);
        }
        UTVideoManager.instance().setCurrentVideoHeight(0);
        UTVideoManager.instance().setCurrentVideoWidth(0);
    ...
    public void onVideoSizeChanged() {
        int mVideoWidth = UTVideoManager.instance().getCurrentVideoWidth();
        int mVideoHeight = UTVideoManager.instance().getCurrentVideoHeight();
    ...
    public int getCurrentPositionWhenPlaying() {
        ...
        if (mCurrentState == CURRENT_STATE_PLAYING || mCurrentState == CURRENT_STATE_PAUSE) {
            try {
                position = (int) UTVideoManager.instance().getMediaPlayer().getCurrentPosition();
    ...
    public int getDuration() {
        int duration = 0;
        try {
            duration = (int) UTVideoManager.instance().getMediaPlayer().getDuration();
        } catch (IllegalStateException e) {
            e.printStackTrace();
            return duration;
        }
    ...
    protected boolean isCurrentMediaListener() {
        return UTVideoManager.instance().listener() != null
                && UTVideoManager.instance().listener() == this;
    }
    ...
    public static boolean backFromWindowFull(Context context) {
        ...
        if (oldF != null) {
            backFrom = true;
            if (UTVideoManager.instance().lastListener() != null) {
                UTVideoManager.instance().lastListener().onBackFullscreen();
            }
        }
        return backFrom;
    }
    ...
    public long getNetSpeed() {
        if (UTVideoManager.instance().getMediaPlayer() != null
                && (UTVideoManager.instance().getMediaPlayer() instanceof IjkMediaPlayer)) {
            return ((IjkMediaPlayer) UTVideoManager.instance().getMediaPlayer()).getTcpSpeed();
        } else {
            return -1;
        }
